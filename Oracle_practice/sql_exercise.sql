...
select employee_id 사원번호, first_name 이름, salary 월급 from employees where department_id = 10;




--사원번호가 7369인 사람 중 이름, 입사일, 부서 번호를 출력하라.
select first_name 이름, hire_date 입사일, department_id 부서번호 from employees where employee_id=7369;
...
select first_name 이름, department_id 부서번호, salary 월급 from employees where hire_date='08/04/21';




--직무가 SA_MAN 아닌 사람의 모든 정보를 출력하라.
select * from employees where job_id not in 'sa_man';
...
select first_name, employee_id, department_id from employees where department_id not in (20, 30);
...
select * from employees where first_name like 'S%t';
...
*/
select first_name, salary, (salary * commission_pct) as 상여, (salary + salary * commission_pct) as 총액 from employees where commission_pct is not null;
...
**/
select first_name, salary, (salary * 0.13) as 보너스금액, department_id from employees where department_id = 10;
...
30번 부서의 연봉을 계산하여 이름, 부서번호, 급여, 연봉을 출력하라. 단 연말에 급여의 150%를 보너스로 지급한다.
   -- 연봉 = sal*12+(sal*1.5)
**/
select first_name, department_id, salary, ((salary * 12) + (salary * 1.5)) as 연봉 from employees where department_id=30;
...
**/
select first_name 이름, salary 급여, round(((salary / 12) / 5), 1) as "시간당 임금" FROM employees where department_id = 20;
...
**/
select first_name as 이름, salary as 급여, round((salary * 0.05), -2) as 회비 from employees where salary >= 1500 and salary <= 3000;
...
select
    ...
    trunc(to_char(sysdate, 'YYYY') - to_char(hire_date, 'YYYY')) 근무년수,
    trunc((sysdate - hire_date) / 30) 근무월수
from employees;
...
**/
select first_name 이름, hire_date 입사일, round(sysdate - hire_date, 0) 근무일수 From employees;
...
입사일을 2012년 7월 5일의 형태로 이름, 입사일을 출력하라.
	...
	-- 'fmYYYY"년" MM"월" DD"일' 
**/
select first_name 이름, to_char(hire_date, 'fmYYYY"년" MM"월" DD"일') 입사일 FRom employees;
...
이름(first_name)의 글자수가 6자이상인 사람의 이름을 앞에서 3자만 구하여 소문자로 이름만을 출력하라.
	...
	-- length(str)  str의 길이
**/
select lower(substr(first_name, 1, 3)) as "이름" from employees where length(first_name) >= 6;
...
**/
select avg(salary) 평균, max(salary) 최고, min(salary) 최저, count(*) 인원수 from employees where department_id = 10;
...
**/
select round(avg(salary), 1) 평균, max(salary) 최고, min(salary) 최저, count(*) 인원수 from employees group by department_id;
...
**/
select department_id 부서번호, round(avg(salary), 1) 평균월급, sum(salary) 전체월급, max(salary) 최고월급, min(salary) 최저월급 from employees group by department_id order by 평균월급 Desc;
