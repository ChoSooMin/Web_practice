...
class StudentService :
    students = [] # 클래스 변수 -> 프로그램 시작시 파일에서 Student 객체 읽어서 초기화 (한번만)
    # -> 매번 초기화 해야 한다면, init.self.students로 초기화한다.
    ...
    # 수강생 삭제 : id 검색해서 수강생 정보 삭제 message 리턴, 존재하지 않는 경우 IDNotFoundException 발생
    def remove(self, id) :
        index = self.is_exist(id)

        if index > -1 : 
            StudentService.students.pop(int(index))
